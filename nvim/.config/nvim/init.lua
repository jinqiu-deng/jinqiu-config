...
-- 把 ANSI 3/11（yellow/bright yellow）改成更耐看的色号
vim.g.terminal_color_3  = "#15C07B"
vim.g.terminal_color_11 = "#15C07B"
...
require("tokyonight").setup({
  ...
  on_highlights = function(hl, c)
    local red = "#FFA07A" -- orangered，比纯红稍柔和
    hl.Comment      = { fg = red }
    hl.comment      = { fg = red }
    hl.perlComment  = { fg = red }
    hl.TSComment    = { fg = red }

    -- 自定义 Copilot 建议高亮组
    -- 提示文本
    hl.CopilotSuggestion = { fg = c.blue1, bg = c.bg_highlight }
    -- 提示的虚拟文本 (virtual text)
    hl.CopilotSuggestionVirtualText = { fg = c.blue1, bg = c.bg_highlight }
  end
})
...
require("colorful-winsep").setup({
  hi = {
    fg = "#8BE9FD",  -- bright cyan for the active split edge
  },
...
require('lualine').setup {
  ...
  winbar           = { lualine_c = { filename_component } },
  inactive_winbar  = { lualine_c = { filename_component } },
}
...
vim.api.nvim_create_autocmd("TermOpen", {
  ...
  callback = function(args)
    ...
    -- 发送 %colors nocolor+回车
    vim.fn.chansend(chan, "%colors nocolor\n")
  end,
})
